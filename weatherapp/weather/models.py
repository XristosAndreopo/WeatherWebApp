...
class FavoriteLocation(models.Model):
    """
    Represents a user’s saved favorite city (with optional country code).
    """
...
class Preference(models.Model):
    ...
    Stores per-user preferences:
      - default_city / default_country: for auto-loading on map/find.
      - default_unit: 'metric' or 'imperial', affects °C vs °F.
      - default_theme: visual theme applied on initial load.
    """
    UNITS_CHOICES = [
        ('metric', 'Metric (°C)'),
        ('imperial', 'Imperial (°F)'),
...
def create_user_preference(sender, instance, created, **kwargs):
    """
    Automatically create a Preference record whenever a new User is created.
    """
    if created:
